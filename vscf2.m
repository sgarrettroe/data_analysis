function scfmodes = vscf2(lmodes,tod,fod,varargin)
...
while ~flag_done
    ...
    for i_mode = 1:n_modes;
        ...
        for ii = 1:n_tod
            ...
            % break loop if not all modes are in the active list
            ...
            % break loop if no mode in the list is the mode we are currently working on
...
ops = {'a','c','q','p','mux','muy','muz','h','h_','q_pow_1','q_pow_2','q_pow_3','q_pow_4'};
n_ops = length(ops);
for i_modes = 1:n_modes
    ...
    for i_ops = 1:n_ops
        if isfield(scfmodes(i_mode),ops{i_ops})
...
fname =  sprintf('q_pow_%i',n(1));
OP = scfmodes_old(i_mode).(fname);
%OP = scfmodes_old(i_mode).q^n(1);
for i_loop = 2:n_unique
    
    tmp = unique_syms(i_loop);
    v = scfmodes_old(tmp).U(:,1);
    fname =  sprintf('q_pow_%i',n(i_loop));
    aux = v'*scfmodes_old(tmp).(fname)*v;
