...
% Gaussian, using the inputs from the external file
    for ij = 1:length(w1)
        out(ij).Fit = fit(w3(:),R(:,ij), F2Gaus_p, ...
...
clear out
CLS.fit.biexp_const = fit(CLS.t2(:), CLS.slope(:), F2Exp, 'StartPoint', [1, 1, 2e3, 2e4,0],...
'lower', [0, 0, 0, 0,0],'Weight',wt);
CLS.fit.biexp = fit(CLS.t2(:), CLS.slope(:), F2Exp_nc, 'StartPoint', [1, 1, 2e3, 2e4],...
'lower', [0, 0, 0, 0],'Weight',wt);
clear wt wt_mean
...
plot(t,CLS.fit.biexp_const(t),'r--');
