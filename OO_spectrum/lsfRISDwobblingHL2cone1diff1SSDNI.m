classdef lsfRISDwobblingHL2cone1diff1SSDNI < lineshapeFunction
    
    properties
        params = struct('Delta_cm',[],'tr1',[],'theta1_deg',[],'tr2',[],'theta2_deg',[],'tr3',[],'T2',[],'ampSSD',[],'tauSSD',[]);
        g;
        c2;
        order;
        tpoints;
    ...
    methods
        ...
        function obj = lsfRISDwobblingHL2cone1diff1SSDNI(params,str,aRFoptions) %constructor function
            if nargin == 0
                super_args = {};
            elseif nargin == 1 
        ...
        function out = makeG(obj)
            ...
            if strcmpi(obj.pol,'para')
                F =@(t, tau) (t-tau).*a.*exp(-tau./tauSSD).*Delta.^2.*obj.R.para(tau,p); %this is the FFCF times (t-tau) to turn a double integral into a single one                
            elseif strcmpi(obj.pol,'perp')
                F =@(t, tau) (t-tau).*a.*exp(-tau./tauSSD).*Delta.^2.*obj.R.perp(tau,p); %this is the FFCF times (t-tau) to turn a double integral into a single one
            else
