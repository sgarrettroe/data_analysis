...
% at the center of the band
lsfParams(1) = fitParamBnd('Delta_cm',   6.25,   0.01,  50,     ''); %total linewidth
lsfParams(2) = fitParamBnd('T2',            2,   0,     50,     ''); %   
lsfParams(3) = fitParamBnd('tr1',           2,   1,     100,    ''); %cone 1 time
...
toc
%% chi by eye?
matrixPlot2DIR(m.s(1).simMatrix(:,:,[1 19 25]),m.s(1).w1_in,m.s(1).w3_in,m.s(1).t2_array([1 19 25]),[1 3],'height_px',100,'zlimit',0.7); %sim
% matrixPlot2DIR(m.s(2).simMatrix,m.s(2).w1_in,m.s(2).w3_in,m.s(2).t2_array,[5 6],'height_px',100,'zlimit',1) %sim
...
zzxx_data(ii).t2 = m.s(2).t2_array(ii).*1000;
  ...
  for ii = 1 : length(t2_array_zzzz)
      I_red_spara(ii) = zzzz_data(ii).R(6,22)./zzzz_data(1).R(6,22);
      I_red_sperp(ii) = zzxx_data(ii).R(6,22)./zzzz_data(1).R(6,22);
  ...
  for ii = 1 : length(t2_array_zzzz)
      I_blue_spara_norm(ii) = zzzz_data(ii).R(20,22)./zzzz_data(1).R(20,22);
      I_blue_sperp_norm(ii) = zzxx_data(ii).R(20,22)./zzzz_data(1).R(20,22);
...
semilogx(t2_array_zzzz,0.4.*L_l{2}(t2_array_zzzz,p))
    xlabel('t_2 (ps)')
    ylabel('anisotropy')
    legend('Experimental','Simulated','L_2')
hold off
...
%% HERE FORWARD IS FOR FITTING TO NEW SSD and T2 RISD MODEL
% close all
clear dyn
...
lsfParams(10) = fitParamBnd('tauSSD'   ,    20,     1,     900,     ''); 
...
% the lsf needs params from the aRFoptions also to set up the numerical integration time
lsf = lsfRISDwobblingHL2cone1diff1SSDNIBnd(lsfParams,'free',aRFoptions);
...
%below should be for perp
aRFoptions.pol = 'perp';
lsf = lsfRISDwobblingHL2cone1diff1SSDNIBnd(lsfParams,'free',aRFoptions); %have to reinitialize this so that the 'perp' get applied
fun_array = {oPerp}; 
dynOther = additionalDynamics(label,fun_array,ind_array);%has no free parameters (gets them from lsf)
aRFoptions.damping = lsf;
aRFoptions.dynOther = dynOther;
...
simanisopeak = squeeze(simaniso(5,7,:))';
figure(30000)
        [C,h]=contourf(cropData(zzzz_data(1),w1,w3).w1,cropData(zzzz_data(1),w1,w3).w3,simaniso(:,:,1),(0:0.005:0.4));
        set(gcf,'PaperUnits','centimeters','PaperPosition',[5.0 5.0 7.5 5.0]);
        title(sprintf('%0.1f ps',t2(1)));
        xlabel('\omega_1/2\pic (cm^{-1})');
        ylabel('\omega_3/2\pic (cm^{-1})');
...
semilogx(t2_array_zzzz,anisopeak,'o')
hold on
box off
semilogx(t2_array_zzzz,0.4.*lsf.L_l{2}(t2_array_zzzz,p),'k-')
