classdef lsfRISDdiffusionNIperptest < lineshapeFunction
    ...
    methods
        function obj = lsfRISDdiffusionNIperptest(params,str,aRFoptions)
            if nargin == 0;
                super_args = {};
            else
                super_args{1} = params;
...
%             F_para =@(t) (3/25).*(11.*exp(-2.*D_m.*t) + 4.*exp(-12.*D_m.*t)) ./ (1 +0.8.*exp(-6.*D_m.*t)); %this is the FFCF
            F_perp =@(t, tau) (t-tau).*(3/25).*(7.*exp(-2.*D_m.*tau) - 2.*exp(-12.*D_m.*tau)) ./ (1 - 0.4.*exp(-6.*D_m.*tau)); %this is the FFCF
